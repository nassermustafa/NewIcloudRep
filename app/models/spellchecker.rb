...
class Spellchecker
  ...
  def edits1(word)
    ...
    return (deletes + transposes + replaces + inserts).to_set.to_a
  end
  ...
  def correct(w)
    if known([w])
      ...
      eds = known(edits1(w))
      unless eds #if no distance 1 edits
        eds = known_edits2(w) #try distance 2 edits
      end
      if (eds)
        eds.sort_by! {|e| 1-@dictionary[e]} #sorts descending frequency of edits
        return eds
      else
        return nil 
      end #endif eds
